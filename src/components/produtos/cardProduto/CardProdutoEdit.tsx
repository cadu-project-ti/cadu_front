...
function CardProduto({ prod }: CardProdutoProps) {
  ...
  return (
    <div className="flex flex-col border-white border-2 bg-white p-1 shadow-md items-center mx-10 md:w-80 md:h-[400px] rounded-lg">
      ...
      <div className="bg-[#19191a] border-white w-full flex justify-center p-4 rounded-t-lg h-40">
        <img src={prod.foto} alt="Imagem do Produto" className="w-28 h-28 object-contain" />
      </div>

      {/* Segunda div com fundo colorido e bordas arredondadas */}
      <div className="w-full p-2 text-center">
        <p className="text-lg text-black font-bold">{prod.nome}</p>
        <p className="line-through text-red-600 font-bold">
          {prod.preco.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
        </p>
        <p className="text-2xl text-[#53c38d] font-bold">
          {novoValor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
        </p>
      </div>

      {/* Botões de editar e deletar */}
      <div className="flex space-x-5 mt-auto mb-2"> {/* mt-auto para empurrar os botões para o final do card */}
        <Link
